...
public class MenuState extends GameState {
	...
	private String[] menu = {"Start", "Controls", "Exit"};
	...
	public MenuState(GameStateManager gsm) {
		...
		try {
			...
			title = ImageLoader.loadImage("/PlayerUI/title.png");
		...
		for (int i = 0; i < menu.length; i++) {
			if (i == currentchoice) {
				if (i == 0) {
					...
					g.drawString(menu[i], 440, 400 + 80 * i);
				}
				else if (i == 1) {
					g.setFont(selectedFont);
					g.setColor(selectedColor);
					g.drawString(menu[i], 390, 400 + 80 * i);
				}
				else if (i == 2) {
					g.setFont(selectedFont);
					g.setColor(selectedColor);
				...
				if (i == 0) {
					...
					g.drawString(menu[i], 460, 400 + 80 * i);
				}
				else if (i == 1) {
					g.setFont(normalFont);
					g.setColor(normalColor);
					g.drawString(menu[i], 420, 400 + 80 * i);
				}
				else if (i == 2) {
					g.setFont(normalFont);
					g.setColor(normalColor);
